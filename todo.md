...
- [x] 시계 방향 점등 — 시계 영역 위에서 마우스 좌→우 이동 시 주황(#FF3B00, 알파 .95)으로 점등+글로우, 우→좌 이동 시 원래 반투명 흰색 복귀. 0.7s 색 전환, rect 히트테스트(pointer-events:none 유지), 데스크탑 전용 (L→R 점등/R→L 소등/재점등/최종색 실측)
